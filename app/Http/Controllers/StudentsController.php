...
class StudentsController extends Controller {
	...
	public function export(){
		...
		$data = array(1 => array ('Roll', 'Full Name','User Name','E-mail','Gender','Address','Mobile No','birthday','Class','Section','password'));
		...
		foreach ($student as $value) {
			...
			$data[] = array ($value->studentRollId, $value->fullName,$value->username,$value->email,$value->gender,$value->address,$value->mobileNo,$birthday,isset($classArray[$value->studentClass]) ? $classArray[$value->studentClass] : "",isset($sectionsArray[$value->studentSection]) ? $sectionsArray[$value->studentSection] : "","");
		}
		...
		foreach ($examMarks as $mark) {
			...
			if($marks[$mark->examId]['data'][$mark->id]['passGrade'] != ""){
				if(intval($marks[$mark->examId]['data'][$mark->id]['totalMarks']) >= intval($marks[$mark->examId]['data'][$mark->id]['passGrade'])){
					$marks[$mark->examId]['data'][$mark->id]['examState'] = "Aprobado";
				}else{
					$marks[$mark->examId]['data'][$mark->id]['examState'] = "Reprobado";
				}
			}
